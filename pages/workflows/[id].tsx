...
function Wizard({
  ...
  async function addWizardStep(type: "visit" | "connect" | "message") {
    setWizardSteps((prev) => {
      ...
      const newStep: WizardStep = { type, delayDaysBefore: isFirst ? 0 : 1, connectNote: "", messageBody: "", templateId: null };
    ...
    for (const ws of wizardSteps) {
      ...
      await fetch(`/api/workflows/${workflowId}/steps`, {
        ...
        body: JSON.stringify({
          ...
          template_id: ws.type === "message" ? (ws.templateId ?? null) : null,
        }),
      });
    }
    setSaving(false);
  ...
  async function launch() {
    ...
    if (prospectMode === "manual" && selectedTargetIds.size === 0) { toast.error("Select at least one prospect"); return; }
    await saveStepsToDB();
    setLaunching(true);
  ...
  const prospectsReady = !!listId && !allBlocked &&
    (prospectMode === "all" || selectedTargetIds.size > 0);
            ...
            <div className="max-w-2xl w-full mx-auto">
              ...
              {page === "prospects" && (
                <div>
                  ...
                  {/* Selection mode — only show once a list is selected */}
                  {listId && !conflictsLoading && !allBlocked && (
                    <div className="mt-2">
                      <p className="text-xs text-base-content/40 mb-2 uppercase tracking-wide">Who to enroll</p>
                      <div className="flex gap-2 mb-4">
                        <button
                          onClick={() => setProspectMode("all")}
                        ...
                        >
                          Manual selection
                          {prospectMode === "manual" && selectedTargetIds.size > 0 && (
                        ...
                        <div className="border border-base-300/50 rounded-xl overflow-hidden">
                          <div className="px-4 py-2.5 bg-base-200 border-b border-base-300/50 flex items-center gap-3">
                            ...
                            <span className="text-xs text-base-content/50">
                              {selectedTargetIds.size === 0 ? "Select all" : `${selectedTargetIds.size} of ${listTargets.length} selected`}
                            </span>
                          </div>
                      ...
                      <div key={idx}>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {ws.type === "message" && ws.messageBody && (
                          ...
                          <div className="border border-t-0 border-primary/20 rounded-b-xl bg-base-200/50 px-5 py-5 mb-1">
                            ...
                            {ws.type === "message" && (
                              <div className="space-y-3">
                                ...
                                {templates.length > 0 && (
                                  <div>
                                    <p className="text-xs text-base-content/40 mb-1">Use a saved template</p>
                                    <select
                                      className="select select-sm w-full"
                                      value={ws.templateId ?? ""}
                                      onChange={(e) => {
                                        const tid = e.target.value ? e.target.value : null;
                                        updateStep(idx, { templateId: tid });
                                      }}
                                    >
                                    ...
                                    {ws.templateId && (
                                      <p className="text-xs text-base-content/30 mt-1">Template will be used. Inline text below is ignored.</p>
                                    )}
                                  </div>
                                )}
                                {/* Inline body */}
                                <div className="flex gap-4">
                                  <div className="flex-1 min-w-0">
                                    ...
                                    <textarea
                                      id={`msg-${idx}`}
                                      className={`textarea textarea-bordered w-full bg-base-300/50 text-sm h-32 resize-none font-mono ${ws.templateId ? "opacity-40 pointer-events-none" : ""}`}
                                      placeholder="Hi {{first_name}}, I noticed..."
                                      value={ws.messageBody}
                                      onChange={(e) => updateStep(idx, { messageBody: e.target.value })}
                                      disabled={!!ws.templateId}
                                    />
                  ...
                  <div className="bg-base-200 border border-base-300/50 rounded-xl p-5 space-y-4 mb-6">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-base-content/40">{selectedList?.target_count} prospects</p>
                      </div>
                    </div>
                    <div className="h-px bg-base-300/50" />
                    <div className="flex justify-between text-sm">
