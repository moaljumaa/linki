...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    const db = getDb();
    const { full_name, linkedin_url, title, company, location, email, list_id } = req.body;
    ...
    try {
      db.prepare(
        `INSERT INTO targets (id, full_name, linkedin_url, title, company, location, email)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      ).run(id, full_name, linkedin_url, title ?? null, company ?? null, location ?? null, email ?? null);
    } catch (e: unknown) {
